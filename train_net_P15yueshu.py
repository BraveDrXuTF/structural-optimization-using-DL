...
def yueShu(x1, x2):
    return 4*x1/math.sqrt(3)+math.sqrt(3)*x2
...
if __name__ == '__main__':
    ...
    for i in range(num_trainsamples):
        ...
        train_data[i][2] = yueShu(train_data[i][0], train_data[i][1])
    X = train_data[:, 0:2]
    ...
    for i in range(num_testsamples):
        ...
        test_data[i][2] = yueShu(test_data[i][0], test_data[i][1])
        
    logdir = './callbacks_15yueshu'
    if not os.path.exists(logdir):
        os.mkdir(logdir)
    output_model_file = os.path.join(logdir,
                                     "mybestmodel15_yueshu.h5")
